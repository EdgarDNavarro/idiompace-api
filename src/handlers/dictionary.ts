...
import { experimental_generateSpeech as speech, generateText } from "ai";
...
export const getTranslation = async (req: Request, res: Response) => {
    const { text, sl, dl } = req.query;
    console.log({ text });

    try {
        const response = await axios.get(`https://ftapi.pythonanywhere.com/translate?sl=${sl}&dl=${dl}&text=${text}`);
...
export const generateElevenLabsTTS = async (req: Request, res: Response) => {
    try {

        const audio = await elevenlabs.textToSpeech.convertWithTimestamps(
            'Nh2zY9kknu6z4pZy6FhD', // voice_id
            {
        ...
        console.log(error);
        res.status(500).json({ error: "Failed to generate ElevenLabs TTS" });
    }
}


export const generateStoryAndAudio = async (req: Request, res: Response) => {
    try {
